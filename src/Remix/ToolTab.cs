...
namespace WikiUtil.Remix
{
    internal class ToolTab(OptionInterface owner) : Tab(owner, "Tools & Keybinds")
    {
        public override void Initialize()
        {
            ...
            var sortedTools = RemixMenu.EnabledConfig
                .Select(x => new KeyValuePair<string, ToolDatabase.ToolType>(x.Key.ToString(), x.Key))
                .OrderBy(x => x.Key, StringComparer.OrdinalIgnoreCase)
                .ToList();
            var inputBeginX = sortedTools.Max(x => LabelTest.GetWidth(Translate(x.Key), false)) + LabelTest.GetWidth(": ", false) + 10f + 34f;
            ...
            {
                y -= 30f;
                var enabledCB = new OpCheckBox(RemixMenu.EnabledConfig[tool.Value], new Vector2(10f, y));
                var toolName = new OpLabel(new Vector2(44f, y), new Vector2(inputBeginX - 44f, 24f), tool.Key, FLabelAlignment.Left, false)
                { verticalAlignment = OpLabel.LabelVAlignment.Center };
                var ctrlCB  = new OpCheckBox(RemixMenu.ControlConfig[tool.Value], new Vector2(inputBeginX,         y));
                var altCB   = new OpCheckBox(RemixMenu.AltConfig[tool.Value],     new Vector2(inputBeginX + 34f,   y));
                var shiftCB = new OpCheckBox(RemixMenu.ShiftConfig[tool.Value],   new Vector2(inputBeginX + 34f*2, y));
                var keyInput = new OpKeyBinder(RemixMenu.KeycodeConfig[tool.Value], new Vector2(inputBeginX + 34f * 3, y), new Vector2(120f, 24f));

                enabledCB.OnValueChanged += OnValueChanged;
                ctrlCB.OnValueChanged += OnValueChanged;
                altCB.OnValueChanged += OnValueChanged;
                shiftCB.OnValueChanged += OnValueChanged;
                keyInput.OnValueChanged += OnValueChanged;

                box.AddItems(enabledCB, toolName, ctrlCB, altCB, shiftCB, keyInput);

                void OnValueChanged(UIconfig config, string value, string oldValue)
                {
                    if (value != oldValue)
                    {
                        RemixMenu.UpdateCacheFor(tool.Value);
                    }
                }
            }
